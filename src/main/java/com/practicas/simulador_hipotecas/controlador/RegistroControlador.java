...
public class RegistroControlador {
	...
	public String verRegistro(Model model) {
		...
		// Si el email ya existe se envía un mensaje de error a la vista
		if (existeEmail != null) {
			model.addAttribute("cliente", cliente);
			model.addAttribute("existeEmail", existeEmail);	
			
		// Si las claves no coinciden se envía un mensaje de error a la vista
	...
	public String submitRegistro(@ModelAttribute("cliente") Cliente cliente, Model model,
		...
		// Comprobar que las claves introducidas sean iguales
